...
function renderAqiList() {
  ...
  for (var city in aqiData) {
    strHTML += '<tr><td>' + city + '</td><td>' + aqiData[city] + '</td>';
    strHTML += '<td><button class="del-btn" data-city="' + city + '">删除</button></td></tr>';
  }
  aqiTable.innerHTML = Object.keys(aqiData).length > 0 ? strHTML : '';
}
...
function delBtnHandle(target) {
  // do sth.
  delete aqiData[target.getAttribute('data-city')];
  renderAqiList();
}
